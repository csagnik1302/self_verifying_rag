...
def generator(prompt, system_prompt, gemini_api_key, generator_model='gemini-3.6-flash'):
    ...
    final_json=[i.model_dump() for i in output]
...
# This ensures that the STRING containing the required json output generated by the model is ready to be used as a python json array (we can then use that readymade json elsewhere as well, without havig through go through the 
